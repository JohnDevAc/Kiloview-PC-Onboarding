...
internal static class AgentConfigurationValidity
{
    ...
    {
        return schema == 1 && Guid.TryParse(endpointId, out var endpoint) && endpoint != Guid.Empty
            && IsValidNetwork(adapterId, address, prefix);
    }

    internal static bool IsValidNetwork(string? adapterId, string? address, int prefix)
    {
        if (!Guid.TryParse(adapterId, out var adapter) || adapter == Guid.Empty || prefix is < 1 or > 30
